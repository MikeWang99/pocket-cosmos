  ...
  sections: [
    {
      ...
      paragraphs: [
        text(
          'A trolley on a low-friction track keeps moving after the push ends. A force changes velocity; it is not needed to maintain constant velocity.',
          '低摩擦轨道上的小车在推力结束后仍会继续运动。力用于改变速度，而不是维持恒定速度。',
        ),
      ],
    },
    {
      ...
      paragraphs: [
        text(
          'Force is a vector. Collinear forces in the same direction add; opposite forces subtract. If the resultant force is zero, velocity remains constant. If it is non-zero, speed and/or direction changes.',
          '力是矢量。同向共线力相加，反向共线力相减。合力为零时速度保持不变；合力不为零时，速率和/或方向发生改变。',
        ),
      ],
      formulas: [formula('Resultant force', '合力', '\\vec F_{\\mathrm{resultant}}=\\sum_i\\vec F_i')],
    ...
    },
    {
      ...
      paragraphs: [
        text(
          'Acceleration is produced by the resultant force, not by one selected force. Its direction is the direction of the resultant force.',
          '产生加速度的是合力，而不是任意选出的某一个力。加速度方向与合力方向相同。',
        ),
      ],
      formulas: [formula('Newton’s second law', '牛顿第二定律', '\\vec F_{\\mathrm{resultant}}=m\\vec a')],
    },
    {
      heading: text('3. Friction, drag and curved motion', '3. 摩擦、阻力与曲线运动'),
      bullets: [
        text('Friction opposes relative motion between surfaces.', '摩擦力阻碍接触面之间的相对运动。'),
        text('Drag opposes motion through a liquid or gas and usually increases with speed.', '阻力阻碍物体在液体或气体中的运动，通常随速率增大。'),
        text('A force perpendicular to velocity changes direction and produces curved motion.', '与速度垂直的力改变运动方向并产生曲线运动。'),
      ],
    },
    {
      ...
      paragraphs: [
        text(
          'Extension is the stretched length minus the original length. In the proportional region, force is proportional to extension. Beyond the limit of proportionality, the graph is no longer straight.',
          '伸长量等于拉伸后的长度减去原长。在正比区域内，力与伸长量成正比；超过正比极限后，图像不再是直线。',
        ),
      ],
      formulas: [
    ...
    },
    {
      ...
      paragraphs: [
        text(
          'A moment depends on force and the perpendicular distance from the pivot to the line of action. Equilibrium requires both zero resultant force and zero resultant moment.',
          '力矩取决于力和转轴到作用线的垂直距离。平衡要求合力为零且合力矩为零。',
        ),
      ],
      formulas: [
        formula('Moment', '力矩', 'M=Fd_{\\perp}'),
        formula('Equilibrium', '平衡条件', '\\sum\\vec F=0,\\qquad \\sum M=0'),
      ],
    ...
    },
    {
      ...
      paragraphs: [
        text(
          'The centre of gravity is the point through which the whole weight may be considered to act. Stability increases when the centre of gravity is lower and the base is wider. Toppling begins when the weight’s line of action falls outside the base.',
          '重心可视为物体全部重量的作用点。重心越低、支撑面越宽，物体越稳定。当重量作用线落到支撑面之外时，物体开始倾倒。',
        ),
      ],
    },
  ],
