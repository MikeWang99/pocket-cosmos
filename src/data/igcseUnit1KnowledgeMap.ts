...
const energyWorkPowerLesson: CurriculumLesson = {
  ...
  sections: [
    ...
    },
    {
      ...
      table: {
        caption: text('Energy Resources Comparison', '能源资源对比'),
        headers: [
          text('Resource', '能源'),
          text('Renewable?', '可再生？'),
          text('Reliability', '可靠性'),
          text('Scale', '规模'),
          text('Environmental Impact', '环境影响'),
        ],
        rows: [
          [
            text('Fossil fuels', '化石燃料'),
            text('No', '否'),
            text('High — dispatchable on demand', '高 — 可按需调度'),
            text('Large (national grid)', '大（国家电网）'),
            text('High CO₂, air pollution, habitat damage', '高 CO₂，空气污染，栖息地破坏'),
          ],
          [
            text('Biofuels', '生物燃料'),
            text('Yes (if sustainably grown)', '是（可持续种植时）'),
            text('Medium — supply-chain dependent', '中 — 依赖供应链'),
            text('Medium', '中等'),
            text('Carbon-neutral in theory; land-use conflict with food', '理论上碳中和；与粮食争地'),
          ],
          [
            text('Hydroelectric', '水力发电'),
            text('Yes', '是'),
            text('High with reservoir storage', '有水库时高'),
            text('Large', '大'),
            text('Floods valleys, displaces communities; no direct emissions', '淹没河谷，迁移居民；无直接排放'),
          ],
          [
            text('Wind', '风能'),
            text('Yes', '是'),
            text('Intermittent — weather dependent', '间歇性 — 依赖天气'),
            text('Medium to large (farms)', '中到大（风电场）'),
            text('No emissions; visual/noise impact, bird/bat risk', '无排放；视觉/噪声影响，鸟类/蝙蝠风险'),
          ],
          [
            text('Tidal', '潮汐能'),
            text('Yes', '是'),
            text('Predictable but tide-locked', '可预测但受潮汐锁定'),
            text('Medium', '中等'),
            text('Affects marine habitats and sediment flow', '影响海洋栖息地和泥沙运动'),
          ],
          [
            text('Wave', '波浪能'),
            text('Yes', '是'),
            text('Intermittent — weather dependent', '间歇性 — 依赖天气'),
            text('Small to medium (emerging)', '小到中等（新兴）'),
            text('No emissions; devices face harsh marine conditions', '无排放；设备面临恶劣海洋环境'),
          ],
          [
            text('Geothermal', '地热能'),
            text('Yes (human timescale)', '是（人类时间尺度）'),
            text('High — baseload', '高 — 基载'),
            text('Medium (location-limited)', '中等（受地点限制）'),
            text('Minimal; drilling may release underground gases', '极少；钻井可能释放地下气体'),
          ],
          [
            text('Nuclear (fission)', '核能（裂变）'),
            text('No', '否'),
            text('High — baseload', '高 — 基载'),
            text('Very large', '极大'),
            text('No CO₂; radioactive waste, accident risk', '无 CO₂；放射性废物，事故风险'),
          ],
          [
            text('Solar cells (PV)', '太阳能电池（光伏）'),
            text('Yes', '是'),
            text('Intermittent — day/night, weather', '间歇性 — 昼夜、天气'),
            text('Small to large (rooftop to farms)', '小到大（屋顶到电站）'),
            text('No emissions; manufacturing chemicals, recycling needed', '无排放；制造化学品，需回收'),
          ],
          [
            text('Solar heating', '太阳能集热'),
            text('Yes', '是'),
            text('Intermittent', '间歇性'),
            text('Small to medium (domestic)', '小到中等（家用）'),
            text('No emissions; only produces heat', '无排放；只产热不发电'),
          ],
        ],
      },
      bullets: [
