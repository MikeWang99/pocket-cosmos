-- Authenticated read RPCs for normalized question data.
-- These let Preview read immutable questions without a service-role secret
-- while normalized tables remain closed to direct client reads.
...
begin
  ...
  return query
  select q.position, q.legacy_question_id, latest.id, latest.metadata
  from public.questions q
  join lateral (
...
begin
  ...
  return query
  select ai.id, ai.position, ai.question_version_id, qv.metadata
