-- Authenticated RPCs for normalized question delivery.
-- These expose only authorized question metadata and keep normalized tables
-- server-only / RLS-locked.
...
revoke all on function public.get_authorized_practice_steps(text) from public, anon;
grant execute on function public.get_authorized_practice_steps(text) to authenticated;

revoke all on function public.get_authorized_assignment_question_steps(uuid) from public, anon;
grant execute on function public.get_authorized_assignment_question_steps(uuid) to authenticated;
