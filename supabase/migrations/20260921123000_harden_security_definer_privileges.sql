-- Security hardening discovered by the Supabase database advisor after Phase 2.
-- Keep anonymous callers away from SECURITY DEFINER helpers and pin the
-- assignment timestamp trigger's search_path.
...
-- Trigger-only helper; never expose through PostgREST.
revoke execute on function public.handle_new_user() from anon, authenticated;

-- Legacy admin helpers must not be anonymously callable.
